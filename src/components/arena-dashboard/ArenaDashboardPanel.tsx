import { BadgeCheck, Flame, Medal, Radio, ShieldCheck, Zap } from 'lucide-react';
...
export function ArenaDashboardPanel({
  managerId,
  overview,
  streak,
}: ArenaDashboardPanelProps) {
  const { t } = useI18n();
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
      <DashboardCard
        accent="from-brass-400/25 via-brass-400/5 to-transparent"
        icon={<Zap className="h-4 w-4" aria-hidden="true" />}
        title={t('arenaDash.position.title')}
      >
        <div className="mt-4">
          {activePosition ? (
            <>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-2xl font-bold tracking-tight text-cream-100">
                    {formatPositionPnl(activePosition)}
                  </div>
                  <div className="mt-1 text-sm text-cream-500">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                <Metric label={t('positions.openSize')} value={formatDUsdc(scaleQuoteAsset(activePosition.openQuantity))} />
                <Metric label={t('positions.expiry')} value={formatTime(activePosition.expiry)} />
              </div>
            </>
          ) : (
            <EmptyState body={t('arenaDash.position.empty')} />
          )}
        </div>
      </DashboardCard>

      <DashboardCard
        accent="from-[#7C5CFF]/30 via-[#FF4FA3]/10 to-transparent"
        icon={<Flame className="h-4 w-4" aria-hidden="true" />}
        title={t('arenaDash.streak.title')}
      >
        <div className="mt-4">
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-sm text-cream-500">
                {streakResolution
                  ? `${streakResolution.consecutiveWins}/${STREAK_TARGET} ${t('arenaDash.streak.wins')}`
                  : t('arenaDash.streak.empty')}
              </div>
            </div>
          ...
          <div className="mt-4 rounded-2xl border border-ink-700/60 bg-ink-950/50 p-3 text-xs leading-5 text-cream-600">
            {t('arenaDash.streak.note')}
          </div>
        </div>
      </DashboardCard>

      <DashboardCard
        accent="from-moss-400/25 via-moss-400/5 to-transparent"
        icon={<Medal className="h-4 w-4" aria-hidden="true" />}
        title={t('arenaDash.leaderboard.title')}
      >
        ...
        <div className="mt-4 text-xs leading-5 text-cream-600">
          {t('arenaDash.leaderboard.note')}
        </div>
      </DashboardCard>

      <DashboardCard
        accent="from-[#FF4FA3]/25 via-[#7C5CFF]/10 to-transparent"
        icon={<ShieldCheck className="h-4 w-4" aria-hidden="true" />}
        title={t('arenaDash.protocol.title')}
      >
        <div className="mt-4 grid gap-3">
          ...
          />
          <div className="flex items-center gap-2 rounded-2xl border border-brass-400/25 bg-brass-400/10 p-3 text-xs text-brass-200">
            <BadgeCheck className="h-4 w-4 shrink-0" aria-hidden="true" />
            {t('arenaDash.protocol.note')}
          </div>
        </div>
      </DashboardCard>

      <DashboardCard
        accent="from-[#7C5CFF]/25 via-[#F6C84C]/5 to-transparent"
        icon={<Radio className="h-4 w-4" aria-hidden="true" />}
        title={t('arenaDash.calls.title')}
      >
        <div className="mt-4">
          <div className="text-2xl font-bold tracking-tight text-cream-100">
            {t('arenaDash.calls.value')}
          </div>
          <div className="mt-2 text-sm leading-6 text-cream-500">
            {t('arenaDash.calls.note')}
          </div>
        </div>
      </DashboardCard>
    </section>
...
function DashboardCard({
  ...
  return (
    <article className="relative overflow-hidden rounded-3xl border border-ink-700/60 glass p-5">
      <div className={`absolute inset-0 bg-gradient-to-br ${accent}`} />
      <div className="relative">
        <div className="flex items-center justify-between gap-3">
          <h3 className="text-sm font-bold text-cream-100">{title}</h3>
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-cream-100/10 text-brass-300">
            {icon}
          </span>
        </div>
        {children}
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className="rounded-full border border-brass-400/35 bg-brass-400/10 px-2.5 py-1 text-xs font-bold text-brass-200">
